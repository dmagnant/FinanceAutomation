...
def getTransactionTotal(dateRange, gnuAccount, mybook):
    ...
    for tr in transactions:
        ...
        for spl in tr.splits:
            if spl.account.fullname == gnuAccount:
                if "Income:" in gnuAccount or "'s Contributions" in gnuAccount: value = -spl.value 
...
def getTotalForEachAccount(accountList, mybook, dateRange, timeframe, month, accounts="Personal"):
    for i in accountList:
        ...
        if timeframe == "Month":    updateSpreadsheet(i, month, totalTransactions, accounts)
        else:                       print(i); print(totalTransactions)

def compileGnuTransactions(account, mybook, dateRange):
    gnuAccount = mybook.getGnuAccount(account)
    total, writeBook = 0, mybook.getWriteBook()
    ...
    if len(accountChildren) > 0:
        for account in accountChildren: total += getTransactionTotal(dateRange, account.fullname, mybook)
    return total
...
def runUpdateGoals(accounts, timeframe, book):
    ...
    openSpreadsheet(driver, 'Asset Allocation', 'Goals') if accounts == "Personal" else openSpreadsheet(driver, 'Home', 'Finances')
    dateRange = getStartAndEndOfDateRange(datetime.today().date(), timeframe)
    incomeAccounts, expenseAccounts, incomeQuarterlyAccounts, expenseQuarterlyAccounts = [], [], [], []
    ...
    incomeAccounts.extend(specificIncomeAccounts);  incomeAccounts.extend(incomeQuarterlyAccounts); incomeAccounts.sort()
    expenseAccounts.extend(commonExpenseAccounts);  expenseAccounts.extend(specificExpenseAccounts);    expenseAccounts.extend(expenseQuarterlyAccounts);   expenseAccounts.sort()
