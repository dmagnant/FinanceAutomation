...
def getTotalForEachAccount(accountList, mybook, dateRange, timeframe, directory, month):
...
def compileGnuTransactions(account, mybook, dateRange):
    def matchAccount():
        match account:
            ...
            case 'Loan Interest':
                return 'Expenses:Personal Loan Interest'
            case 'Loan Principle':
...
def runUpdateGoals(accounts, timeframe):
    ...
    mybook = openGnuCashBook('Finance', True, True) if accounts == 'p' else openGnuCashBook('Home', False, False)
    ...
    getTotalForEachAccount(incomeAccounts, mybook, dateRange, timeframe, directory, dateRange[1].month)
    getTotalForEachAccount(expenseAccounts, mybook, dateRange, timeframe, directory, dateRange[1].month)

if __name__ == '__main__':
    accounts = 'p' # p (personal) or j (joint)
