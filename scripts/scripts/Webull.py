...
def webullClosePopUps(driver):
    driver.getElementAndClick('xpath', '/html/body/div[5]/div/div/div[1]/button/svg', wait=0.1) # close 
    driver.getElementAndClick('xpath', '/html/body/div[4]/svg', wait=0.1) # close
...
def parseWebullADDescription(rawDescription):
    if ('Bought' in rawDescription or 'Sold' in rawDescription):
        if '.000' not in rawDescription and '.500' not in rawDescription:
    ...
    description = rawDescription.replace('Bought ', '').replace('Sold ', '').replace('.000', '').replace('00', '')
...
def getFromAccountDetailsPage(driver, dateRange):
    if datetime.today().weekday() != 6 and datetime.today().weekday() != 0:
    ...
    while True:
        adRow+=1
        adRawDateTime = driver.getElementText('xpath', f"{accountDetailsTablePathPrefix}{str(adRow)}]/td[3]") # Date
        print(f'Row: {adRow} - date found: {adRawDateTime}')
        print(f'mindate: {dateRange["startDate"]} - maxdate: {dateRange["endDate"]}')
        if adRawDateTime:
            ...
            if adDateTime.date() >= dateRange['startDate'] and adDateTime.date() <= dateRange['endDate']:
                amount = float(driver.getElementText('xpath', f"{accountDetailsTablePathPrefix}{str(adRow)}]/td[4]").replace('+','').replace(',','')) # Amount
                rawDescription = driver.getElementText('xpath', f"{accountDetailsTablePathPrefix}{str(adRow)}]/td[2]") # Description
                print(f'Row: {adRow} - description found: {rawDescription}')
...
def loadFullOHPage(driver, pathPrefix):
    print('Loading full order history page')
    row = 2
    iterate = 2
    lastRow = 3
    while True:
        row+=iterate
        ohRawDateTime = driver.getElement('xpath',f"{pathPrefix}{str(row)}]/td[9]", wait=0.5) # filled time
        if ohRawDateTime == False:
            if (row - lastRow) > iterate:
                print(f'Row: {row} - date not found')
                break
            else:
                driver.getElementAndSendKeys('xpath', '/html/body', Keys.END)
        else:
            lastRow = row
    driver.getElementAndSendKeys('xpath', '/html/body', Keys.HOME)
    print('Full order history page loaded')
...
def getFromOrderHistoryPage(driver, dateRange):
    ...
    if datetime.today().weekday() == 6 or datetime.today().weekday() == 0: # if Sunday or Monday, select 1M timeframe to be able to capture Friday transactions
        time.sleep(0.5)
    ...
    while True:
        row+=1
        averagePrice = 0
        ohRawDateTime = driver.getElementText('xpath',f"{orderhistoryTablePathPrefix}{str(row)}]/td[9]", wait=2) # filled time
        if ohRawDateTime == False:
            print(f'Row: {row} - date not found')
            break
        elif ohRawDateTime != '':   
            ...
            try:
                ...
                row-=1 # reset row to 1, and try again
                continue
            except Exception:
                print(f'Row: {row} - date is blank')
                continue
        if ohDateTime.date() == datetime.today().date() or (ohDateTime.date().weekday() == 4 and datetime.today().date() in [ohDateTime.date() + timedelta(days=i) for i in range(3)]):
            ...
            actionChains.context_click(driver.getElement('xpath', f"{orderhistoryTablePathPrefix}{str(row)}]/td[3]")).perform() # right-click on row
            time.sleep(0.5)
            driver.getElementAndClick('xpath', "/html/body/div[3]/div/div/div/div[1]/div[2]/div/div/div/button[1]/span") # Order Details
            odRow = 5
            orderDetailsPathPrefix = "/html/body/div[3]/div/div/div[2]/div/div["
            while True:
                fee = 0
                odRow+=1
                feeDescription = driver.getElementText('xpath', f"{orderDetailsPathPrefix}{str(odRow)}]/div[1]", wait=1)
                if feeDescription:
                    if feeDescription == 'Transaction Fee':
                        feeText = driver.getElementText('xpath', f"{orderDetailsPathPrefix}{str(odRow)}]/div[2]", wait=1).replace('$', '')
                        if feeText != '--':
...
def cycleThroughWebullADTransactions(accountDetails, WebullActivity, ohTransaction=None):
    ADspecificTransactions = []
    for ad in accountDetails:
        if 'Interest' in ad[1] or 'dividend' in ad[1].lower() or 'premium fee' in ad[1].lower():
            ad.append(0)
            csv.writer(open(WebullActivity, 'a', newline='')).writerow(ad)
            ADspecificTransactions.append(ad)
        if ohTransaction:
            adAmount = float(ad[2])
            if ad[0] == ohTransaction[0] and ad[1] == ohTransaction[1]:
                ohAmountNet = float(ohTransaction[2]) - float(ohTransaction[3])
                if ohAmountNet != adAmount:
                    ohTransaction[2] = round(adAmount + float(ohTransaction[3]), 2)
                csv.writer(open(WebullActivity, 'a', newline='')).writerow(ohTransaction)
                break
...
def captureWebullTransactions(driver, dateRange):
    ...
    orderHistory = list(csv.reader(open(orderHistoryTransactionsFile), delimiter=','))
    if len(orderHistory) > 0:
        for oh in orderHistory:
            accountDetails = cycleThroughWebullADTransactions(accountDetails, WebullActivity, oh)
    else:
        accountDetails = cycleThroughWebullADTransactions(accountDetails, WebullActivity)
...
def importWebullTransactions(accounts, WebullActivity, book, gnuCashTransactions):
    ...
    for row in csv.reader(open(WebullActivity), delimiter=','):
        ...
        elif 'premium fee' in description.lower():
...
def getWebullDuplicateTransactions(account, book, dateRange):
    ...
    for tr in gnuCashTransactions:
        ...
        for spl in tr.splits:
            if spl.account.fullname == book.getGnuAccountFullName('Bank Fees'):
                fee = spl.value
        if key in transaction_counts:
            transaction_counts[key] += 1  # Increment count if the key already exists
            # Keep the transaction with the highest fee
            if fee > transaction_details[key]['fee']:
                transaction_details[key] = {'transaction': tr, 'fee': fee}
        else:
    ...
    for key, count in transaction_counts.items():
        if count > 1:  # Check if the transaction appears more than once
            ...
            # Delete all duplicates except the one with the highest fee
            highest_fee_transaction = transaction_details[key]['transaction']
            for tr in gnuCashTransactions:
